    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for i in range(len(orig_data.keys())):
            ...
            for obj_name in env.sim.model.body_names:
                if not ("world" in obj_name or "table" in obj_name or "robot" in obj_name or "gripper" in obj_name or "mount" in obj_name):
                    initial_obj_pos[obj_name] = env.sim.data.get_body_xpos(obj_name).copy()
            ...
            for _, action in enumerate(orig_actions):
                ...
                finger1_collisions.append(obs["finger1_collision"])
                finger1_pad_collisions.append(obs["finger1_pad_collision"])
                finger2_collisions.append(obs["finger2_collision"])
                finger2_pad_collisions.append(obs["finger2_pad_collision"])
            ...
            if done:
                ...
                for obj_name in env.sim.model.body_names:
                    if not ("world" in obj_name or "table" in obj_name or "robot" in obj_name or "gripper" in obj_name or "mount" in obj_name):
                        final_obj_pos[obj_name] = env.sim.data.get_body_xpos(obj_name).copy()
                                    
                moved_objects = []
                
                for obj_name in initial_obj_pos:
                    dist = np.linalg.norm(final_obj_pos[obj_name] - initial_obj_pos[obj_name])
                    if dist > 1e-2: # 1cm
                ...
                # forceの追加
                obs_grp.create_dataset("finger1_collision", data=np.stack(finger1_collisions, axis=0))

                obs_grp.create_dataset("finger1_pad_collision", data=np.stack(finger1_pad_collisions, axis=0))

                obs_grp.create_dataset("finger2_collision", data=np.stack(finger2_pad_collisions, axis=0))
        ...
        if task.name not in task_desired_goals:
            task_desired_goals[task.name] = None
        task_desired_goals[task.name] = desired_goals
        
        print("task_desired_goals?", task_desired_goals)

    # JSON ファイルに保存
    with open('task_desired_goals.json', 'w') as f:
        json.dump(task_desired_goals, f, indent=4)

    print("Saved task_desired_goals to task_desired_goals.json")
